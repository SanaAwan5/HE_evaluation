...
class Gradient_List():
    ...
    def generate_gradient_avg(self,agg_addr,transform_key, alpha = 1):
        ...
        e_gen_grad_avg=time.time()
        print("time for avg_gen", e_gen_grad_avg - s_gen_grad_avg)
        ...
        e_avg_write_2_chain=time.time()
        print("avg_write_time_to_chain", e_avg_write_2_chain - s_avg_write_2_chain)
...
class Model():
    ...
    def evaluate_gradient_from_avg(self,addr,agg_addr,avg,transform_key,prikey, pubkey, inputs, targets, alpha=1):
        ...
        new_model_error = candidate.evaluate(inputs, targets)
        e_eval_time=time.time()
        print("model eval time",e_eval_time - s_eval_time)

        tx = self.repo.get_transaction(from_addr=addr)
        ipfs_address = self.repo.ipfs.store(candidate.encrypt(pubkey))
        #length_ = length - 1
        tx.evalGradientfromAvg(self.model_id, new_model_error,IPFSAddress().to_ethereum(ipfs_address))
